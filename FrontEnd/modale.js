...
async function deleteWork(workId) {
  try {
    const response =await fetch(`https://portfolio-architecte-sophie-bluel-cx5e.onrender.com/api/works/${workId}`, {
      method: "DELETE",
...
async function submitHandler(event) {
  ...
  try {
    let response = await fetch(`https://portfolio-architecte-sophie-bluel-cx5e.onrender.com/api/works`, {
      method: "POST",
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      body: formData,
    });
